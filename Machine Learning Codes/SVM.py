...
# X_train_data = pd.concat(li, axis=0, ignore_index=True).loc[:, 'f1ch1': 'f4ch2']
# X_test_data = pd.read_csv(os.path.join('Dataset', files[11])).loc[:, 'f1ch1': 'f4ch2']
# y_train_data = (pd.concat(li, axis=0, ignore_index=True).loc[:, 'Category']).astype(np.float64)
# y_test_data = (pd.read_csv(os.path.join('Dataset', files[11])).loc[:, 'Category']).astype(np.float64)
